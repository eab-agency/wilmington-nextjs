...
const ScrollButtons = ({ scrollElement, className }) => {
  ...
  return (
    <div className={`${styles.scrollButtonsCotainer} ${className}`}>
      <button onClick={() => scrollLeft()} type='button' className={styles.leftArrow}>Scroll Left</button>
      <button onClick={() => scrollRight()} type='button' className={styles.rightArrow}>Scroll Right</button>
    </div>
  );
}
